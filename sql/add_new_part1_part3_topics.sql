...
-- Music (NEW questions for existing topic)
-- First check if Music topic exists, if not create it
INSERT INTO topics (title, part, questions, estimated_time) VALUES
('Music', 'part1', 
...
-- Delete existing Music questions if any, then insert new ones
...
-- Weather (update existing Weather topic with new questions)
DELETE FROM questions 
WHERE topic_id IN (SELECT id FROM topics WHERE title = 'Weather' AND part = 'part1');

UPDATE topics SET questions = 
...
-- Sports (update existing Sports topic with new questions)
DELETE FROM questions 
...
-- Note: Sports exists as Sports II in original, so this creates a new Sports topic
...
-- Shopping (update existing with new questions)
DELETE FROM questions 
WHERE topic_id IN (SELECT id FROM topics WHERE title = 'Shopping' AND part = 'part1');

UPDATE topics SET questions = 
'1. Do you enjoy shopping in malls or online?
...
-- Pets (update existing Pets topic - note: might be Pets II)
INSERT INTO topics (title, part, questions, estimated_time) VALUES
('Pets', 'part1', 
